            ...
            <div key={b.id} className="glass-panel p-4 sm:p-5 space-y-3">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 bg-sky-500/10 text-sky-400 rounded border border-sky-500/20 shrink-0"><MapPin className="w-5 h-5" /></div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs p-2.5 rounded bg-slate-950/40 border border-slate-800/60">
                <div className="text-slate-400"><span className="font-semibold text-slate-300">Population:</span> <span className="font-bold text-slate-200">{b.population?.toLocaleString() || 'N/A'}</span></div>
                <div className="text-slate-400"><span className="font-semibold text-slate-300">Status:</span> <span className="text-emerald-400 font-bold">{b.status}</span></div>
                <div className="text-slate-400"><span className="font-semibold text-slate-300">Lat:</span> <span className="text-slate-300 font-mono text-[11px]">{b.latitude}</span></div>
                <div className="text-slate-400"><span className="font-semibold text-slate-300">Lng:</span> <span className="text-slate-300 font-mono text-[11px]">{b.longitude}</span></div>
              </div>
              <div className="flex gap-2 pt-1 border-t border-slate-800/60">
                <button onClick={() => openEdit(b)} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold transition">
