        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5">
          {centers.map((c) => {
            ...
            return (
              <div
                key={c.id}
                className="glass-panel p-4 sm:p-5 space-y-3.5 sm:space-y-4"
              >
                <div className="flex items-start justify-between gap-2.5">
                  <div className="flex items-start gap-2.5 sm:gap-3 min-w-0 flex-1">
                    <div className="p-2 sm:p-2.5 bg-emerald-500/10 text-emerald-400 rounded border border-emerald-500/20 shrink-0 mt-0.5">
                ...
                {c.description ? (
                  <p className="text-xs text-slate-300 line-clamp-2 bg-slate-950/40 p-2.5 rounded border border-slate-800/60 leading-relaxed">
                    {c.description}
                  </p>
                ) : null}

                <div className="flex items-center gap-2 text-xs text-slate-400 border-t border-slate-800/60 pt-2.5">
                  <Phone className="w-3.5 h-3.5 text-sky-400 shrink-0" />
                  <span className="truncate">
                    {c.contactPerson || "Camp Manager"} —{" "}
                    <span className="font-mono">{c.contactPhone || "N/A"}</span>
                  </span>
                </div>
                ...
                <div className="flex gap-2 pt-1">
                  <button
                    onClick={() => openEdit(c)}
                    className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold transition"
                  >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                placeholder="e.g. San Agustin, Bagsangan, Monbon"
                className="w-full px-3 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-slate-100 focus:outline-none focus:border-sky-500 transition"
              />
            </div>
