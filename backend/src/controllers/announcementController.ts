...
export class AnnouncementController {
  private static async getRegisteredTokens(): Promise<string[]> {
    ...
    if (db) {
      try {
        ...
        console.warn(
          "[AnnouncementController] Firestore fetch push_tokens warning:",
          err,
        );
      }
    }
  ...
  public static ensure5W1HContent(item: any): string {
    ...
    const dateStr = item.eventDate
      ? `${item.eventDate}${item.startTime ? ` – ${item.startTime}` : ""}`
      : "Kasalukuyan at Agaran";
    const brgyStr =
      Array.isArray(item.affectedBarangays) && item.affectedBarangays.length > 0
    ...
    let desc =
      raw ||
      "Sundin ang mga opisyal na tagubilin ng MDRRMO at lokal na pamahalaan.";
    ...
    if (
      ...
      why =
        "Banta ng malalakas na buhos ng ulan at posibleng pagbaha dulot ng sama ng panahon";
      desc =
        "Manatili sa loob ng ligtas na tahanan at subaybayan ang mga susunod na opisyal na abiso.";
    } else if (cat.includes("kuryente") || cat.includes("power")) {
      what = `SORECO II Scheduled Power Interruption: ${title}`;
      who =
        "Lahat ng residente, establisyemento, at konsumidores sa apektadong feeder";
      why =
        "Pagsasaayos ng mga linya ng kuryente at clearing ng mga nakalaylay na sanga";
      desc =
        "I-charge nang maaga ang mga flashlights, powerbanks, at emergency devices bago ang brownout.";
      ...
      why =
        "Tulong at suportang pangkagipitan mula sa Pamahalaang Bayan at DSWD";
      desc =
        "Magdala ng Valid ID o Barangay Certificate of Residency sa distribution site.";
      ...
      desc =
        "Mag-ipon nang sapat na malinis na tubig para sa inumin at gamit sa bahay.";
    }
      ...
      if (db) {
        ...
        const items = snapshot.docs.map((d) => {
          ...
          const formattedContent =
            AnnouncementController.ensure5W1HContent(data);
  ...
  public static async create(req: Request, res: Response) {
    try {
      ...
      const finalImageUrl =
        imageUrl ||
        photoUrl ||
        (Array.isArray(photos) && photos.length > 0 ? photos[0] : "");
      ...
      if (db) {
        ...
        if (
          finalImageUrl &&
          !finalImageUrl.includes("unsplash.com/photo-default")
        ) {
      ...
      if (tokens.length > 0) {
        ...
        await ExpoPushService.sendToTokens(
          ...
          newAnnouncement.content
            ? newAnnouncement.content.length > 130
              ? `${newAnnouncement.content.substring(0, 130)}...`
              : newAnnouncement.content
            : "Pindutin upang mabasa ang buong anunsyo.",
          {
            type: "ANNOUNCEMENT",
            announcementId: id,
      ...
      return res.status(201).json({
        message:
          "Announcement posted and push notifications broadcasted successfully",
  ...
  public static async update(req: Request, res: Response) {
    try {
      ...
      if (db) {
        ...
        emitRealtimeEvent("ANNOUNCEMENTS_CHANGED", {
          action: "UPDATE",
          id,
          announcement: { id, ...doc.data(), ...updatedData },
        });
        return res.json({
